...
class GoDaddyPayInPersonGateway extends AbstractPaymentGateway
{
    ...
    public function process_admin_options()
    {
        ...
        if ($result) {
            $this->maybeTriggerPushSync($pushWasEnabled);
            $this->maybeTriggerPullSync($pullWasEnabled);
        }
    ...
    /**
     * Triggers a push sync if previously disabled.
     *
     * @param bool $wasEnabled
     */
    protected function maybeTriggerPushSync(bool $wasEnabled)
    {
        if ($wasEnabled || 'yes' !== $this->get_option('sync_push_enabled')) {
            return;
        }
     ...
     * @param bool $wasEnabled
     */
    protected function maybeTriggerPullSync(bool $wasEnabled)
    {
        if ($wasEnabled || 'yes' !== $this->get_option('sync_pull_enabled')) {
            return;
        }
