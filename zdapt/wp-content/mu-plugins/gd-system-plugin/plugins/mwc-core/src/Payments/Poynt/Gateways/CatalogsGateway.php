...
class CatalogsGateway extends AbstractGateway
{
    ...
    {
        if (empty($products) || empty($productsNotInCatalog = $this->getProductsNotInCatalog($products, $this->get($catalogId, false)))) {
            return;
        }

        // TODO: consider converting the response back to the Catalog object and returning it {@itambek 2022-02-11}
        $request = CatalogRequest::getNewInstance($catalogId)
            ->setMethod('PATCH')
            ->setBody(
                array_map(static function (Product $product) {
                    return [
                        'op'    => 'add',
                        'path'  => '/products/-',
                        'value' => [
