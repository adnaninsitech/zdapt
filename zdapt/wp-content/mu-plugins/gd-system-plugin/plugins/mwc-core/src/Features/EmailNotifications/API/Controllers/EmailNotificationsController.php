...
class EmailNotificationsController extends AbstractController implements ComponentContract
{
    ...
    public function getItemData(WP_REST_Request $request)
    {
        try {
            $emailNotification = $this->maybeAdjustEmailNotificationObjectForRequest(
                $this->getEmailNotificationDataStore()->read(StringHelper::sanitize($request->get_param('emailNotificationId'))),
                $request
            );

            $response = $this->buildItemData($this->getEmailBuilder($emailNotification)->build());
     ...
     *
     * @TODO MWC-4814 - Create an adapter to replace this method (and all associated methods it uses) to reduce duplication with EmailService::buildMutation() {agibson 2022-03-10}
     *
    ...
    {
        return [
            'emailEventName' => $renderableEmail->getEmailName(),
            'siteId'         => ManagedWooCommerceRepository::getSiteId(),
            'emailMessage'   => ArrayHelper::combine([
                'sender' => [
                    'name'    => $renderableEmail->getFromName(),
                    'address' => $renderableEmail->getFrom(),
                ],
                'toRecipients' => array_map(static function (string $to) : array {
                    return ['address' => $to];
                }, ArrayHelper::wrap($renderableEmail->getTo())),
                'subject'      => $renderableEmail->getSubject(),
                ...
                }, array_keys($renderableEmail->getHeaders()), array_values($renderableEmail->getHeaders())),
                'attachments' => $this->getItemAttachmentData($renderableEmail),
            ], $this->getItemDataBodyFields($renderableEmail)),
            'templateParamsJson' => $renderableEmail->getVariables(),
        ];
    }

    /**
     * Retrieves the attachment data for an item.
     *
     * @TODO Move to an adapter in MWC-4814 {agibson 2022-03-11}
     *
     * @param RenderableEmail $renderableEmail
     * @return array
     * @throws ReflectionException
     */
    protected function getItemAttachmentData(RenderableEmail $renderableEmail) : array
    {
        $attachmentsData = [];
        foreach ($renderableEmail->getAttachments() as $attachmentFile) {
            try {
     ...
     */
    protected function getItemDataBodyFields(RenderableEmail $renderableEmail) : array
    {
        return [$renderableEmail->getBodyFormat() => $renderableEmail->getBody()];
    }
    ...
    public function shouldSendItem(WP_REST_Request $request)
    {
        try {
            ...
            $response = [
                'sendNow' => true, // by default all email notifications should be sent
            ...
            if (is_a($emailNotification, DelayableEmailNotificationContract::class)) {
                ...
                $response['retryAt'] = $sendAt ? $sendAt->getTimestamp() : null;
            }
    ...
    protected function maybeUpdateDelayableSettings(DelayableEmailNotificationContract $emailNotification, WP_REST_Request $request)
    {
        ...
        if ($unit = $request->get_param('sendDelayUnit')) {
            $emailNotification->setDelayUnit(StringHelper::sanitize($unit));
        }
    }
