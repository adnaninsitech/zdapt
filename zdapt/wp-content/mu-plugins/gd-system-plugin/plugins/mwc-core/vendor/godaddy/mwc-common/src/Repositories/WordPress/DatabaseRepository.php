...
class DatabaseRepository
{
    ...
    public static function createTable(string $tableName, array $columns, array $indexes = [], array $foreignKeys = [])
    {
        if (static::tableExists($tableName)) {
            throw new BaseException("Table {$tableName} already exists");
        }
        ...
        if (! static::tableExists($tableName)) {
            throw new BaseException("Table ${tableName} was not created, check your PHP error log for more information");
        }
    }
    ...
    {
        if (! static::tableExists($tableName)) {
            throw new BaseException("Table {$tableName} does not exist");
        }
        ...
        if (false === $deleted) {
            throw new BaseException("Table {$tableName} was not deleted, check your PHP error log for more information");
        }
    }
     ...
     * @throws BaseException if the row could not be inserted
     */
    public static function insert(string $table, array $data, $format = null) : int
    {
        ...
        if ($wpdb->last_error || false === $result || empty($wpdb->insert_id)) {
            throw new BaseException($wpdb->last_error ?? 'Could not insert row.');
        }
     ...
     * @throws BaseException on errors
     */
    public static function replace(string $table, array $data, $format = null) : int
    {
        ...
        if ($wpdb->last_error || ! is_numeric($result)) {
            throw new BaseException($wpdb->last_error ?? 'Could not replace row(s).');
        }
    ...
    public static function update(string $table, array $data, array $where, $format = null, $formatWhere = null) : int
    {
        ...
        if ($wpdb->last_error || ! is_numeric($result)) {
            throw new BaseException($wpdb->last_error ?? 'Could not update row(s).');
        }
    ...
    public static function delete(string $table, array $where, $format = null) : int
    {
        ...
        if ($wpdb->last_error || ! is_numeric($result)) {
            throw new BaseException($wpdb->last_error ?? 'Could not delete row(s).');
        }
